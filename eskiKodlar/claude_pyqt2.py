...
class Worker(QThread):
    ...
    def run(self):
        try:
            ...
            # Otomatik keskinleştirme uygula
    ...
    def sharpen_edges(self, image):
        """Alpha kanalını otomatik keskinleştirerek bulanıklığı azaltır"""
        if image.mode != 'RGBA':
            return image
        ...
        # Adaptif eşik değeri hesapla (ortalama alpha değerine göre)
        non_zero_alpha = alpha[alpha > 0]
        if len(non_zero_alpha) > 0:
            avg_alpha = np.mean(non_zero_alpha)
            # Ortalamaya göre dinamik eşik (genelde 120-160 arası olur)
...
class BackgroundRemoverApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        tips_label = QLabel(
            ...
            "• Fotoğraflar için <b>u2net</b> veya <b>silueta</b><br>"
            "• Küçük resimler otomatik büyütülür<br>"
            "• Kenarlar otomatik keskinleştirilir"
        )
    ...
    def process_image(self):
        ...
        self.status_label.setText(f"⏳ İşleniyor... Model: '{model_adi}'")
        self.status_label.setStyleSheet("color: orange; padding: 8px; font-weight: bold; font-size: 13px;")
        self.process_button.setEnabled(False)
        self.save_button.setEnabled(False)
        QApplication.processEvents()
